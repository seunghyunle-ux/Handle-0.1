...
logoutBtn.onclick = async ()=>{
  try{
    // ✅ ensure latest device pushes state before sign out (sync across devices)
    if(window.MAR_SYNC && typeof window.MAR_SYNC.flush === "function"){
      await window.MAR_SYNC.flush();
    }
    await signOut(auth);
  }catch(e){
    console.error(e);
...
window.MAR_APP = {
  ...
  // ✅ QR/외부 모듈에서 상태 변경을 '저장+동기화 트리거'까지 한 번에 처리
  commitState: (next, reason)=>{
    state = migrateState(next || {patients:{}});
    // saveState는 wrapper로 __rev/__updatedAt 갱신 + sync onLocalChange emit
    saveState();
    renderAll();
    // reason은 디버그용(선택)
    // __emitLocalChange는 saveState 내부에서 호출됨
  },
  ...
  // ✅ QR 스캔 등에서 환자를 "클릭 없이" 선택할 수 있도록 노출
  selectPatient: (name)=>{
    name = (name===null||name===undefined) ? "" : String(name).trim();
    if(!name) return;
    if(!state || !state.patients || !state.patients[name]) return;
    selectedPatient = name;
    renderAll();
  },
