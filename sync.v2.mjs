...
(async function main(){
  ...
  // Expose manual flush for logout/unload (last-logout-wins)
  window.MAR_SYNC = window.MAR_SYNC || {};
  window.MAR_SYNC.flush = async ()=>{
    try{
      if(!auth.currentUser) return;
      if(!facilityCode) return;
      // allow flush even before first remote snapshot
  ...
  setInterval(async ()=>{
    ...
    unsubPatients = onSnapshot(
      ...
      async (snap)=>{
        ...
        // Mark remote ready on first snapshot so local changes can push even if remote isn't newer
        remoteReady = true;
