...
(async function main(){
  ...
  function applyRemoteSnapshot(snap){
    ...
    // Remote snapshot is older or same -> usually ignore (prevents delete/room/mrn revert)
    // However, some browsers/networks can get "stuck" with a higher local rev and never pull.
    // If local has been idle for a bit and remote patients differ, prefer remote to recover sync.
    const lastLocalIdleMs = Date.now() - lastLocal;
    try{
      const localPatientsJson = JSON.stringify((local.patients || {}));
      const remotePatientsJson = JSON.stringify((next.patients || {}));
      const differ = (localPatientsJson !== remotePatientsJson);

      // If no local write recently and remote differs, apply remote even if rev looks older.
      if(differ && lastLocalIdleMs > 5000){
