...
async function initScanUI(){
  ...
  async function refreshScanButtonAccess(){
    try{
      ...
      const allowed = (!role) || (role === "admin" || role === "nurse");
      scanBtn.disabled = !allowed;
      scanBtn.title = allowed ? "Open Scan" : "Scan: nurse/admin만 접근 가능";
    }catch(_e){
      // If role lookup is blocked by Firestore rules, fail-open so Scan works across devices.
      scanBtn.disabled = false;
      scanBtn.title = "Open Scan";
    }
  }
  // removed periodic role polling (caused flicker on some devices)
