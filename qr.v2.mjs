...
async function initScanUI(){
  ...
  function normalizeMedName(s){
    return String(s||"")
      ...
      .replace(/\b\d+(?:\.\d+)?\s*(mg|mcg|g|ml|l|unit|units|iu|meq|mmol)\b/g, " ") // remove common dose
  ...
  function resolveMedKey(patient, scannedName){
    ...
    for(const k of keys){
      ...
      if(!nk) continue;
      if(nk === target) matches.push(k);
    }
    if(matches.length === 1) return matches[0];

    // fallback: prefix match (only if unique)
    matches = [];
    for(const k of keys){
      const nk = normalizeMedName(k);
      if(nk && (nk.startsWith(target) || target.startsWith(nk))) matches.push(k);
    }
    if(matches.length === 1) return matches[0];
