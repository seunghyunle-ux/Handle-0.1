...
function __normName(v){
  return String(v||"").trim().replace(/\s+/g, " ").toLowerCase();
}
function patientKey(p){
  if(!p) return "";
  // NOTE: used only for comparing (not for indexing state). Keep comparison tolerant.
  return `${__normName(p.name)}|${__normId(p.room)}|${__normId(p.mrn)}`;
}
...
function todayKey(){
  // ✅ Use LOCAL date (not UTC) so it matches app.v2.mjs day keys
  const d = new Date();
...
async function initScanUI(){
  ...
  function resetCtx(){
    ...
    ctx.patient = null;
    ctx.patientSetAt = 0;
    setStatus(scanStatus, "환자 QR을 먼저 스캔하세요.");
  ...
  async function startCamera(){
    ...
    await camera.start(
      ...
      { fps: 10, qrbox: 250 },
      async (decodedText)=>{
        // Stop immediately after a successful decode to avoid double triggers
        await stopCamera(true);
        await handleDecoded(decodedText, "camera");
        // Allow user to tap Next scan (acts like "찰칵/다음")
  ...
  function findPatientInState(p){
    ...
    if(!st || !st.patients) return null;

    const nameRaw = String(p?.name||"").trim();

    // 1) exact name key
    if(nameRaw && st.patients[nameRaw]) return { st, patient: st.patients[nameRaw], name: nameRaw };

    // 1b) normalized name match (handles spacing/hidden chars/case)
    const nameNorm = __normName(nameRaw);
    if(nameNorm){
      for(const key of Object.keys(st.patients)){
        if(__normName(key) === nameNorm){
          return { st, patient: st.patients[key], name: key };
        }
      }
    }

    // 2) match by MRN (best) or room (weaker)
    const mrnQ  = __normId(p?.mrn);
    const roomQ = __normId(p?.room);

    let roomHit = null;
    if(mrnQ || roomQ){
      for(const key of Object.keys(st.patients)){
        const pp = st.patients[key] || {};
        const mrnP  = __normId(pp.mrn);
        const roomP = __normId(pp.room);

        if(mrnQ && mrnP && mrnQ === mrnP){
          return { st, patient: st.patients[key], name: key };
        }
        if(!roomHit && roomQ && roomP && roomQ === roomP){
          roomHit = { st, patient: st.patients[key], name: key };
        }
      }
    }
    if(roomHit) return roomHit;

    // 3) last resort: if exactly one patient exists, assume it
    const keys = Object.keys(st.patients);
    if(keys.length === 1){
      const only = keys[0];
      return { st, patient: st.patients[only], name: only };
    }
  ...
  function normalizeMedName(s){
    return String(s||"")
      ...
      // remove common route/form words before stripping punctuation
      .replace(/\b(po|oral|iv|im|sc|sq|subcut|subcutaneous|pr|sl|top|topical|inh|inhaled|neb|nebulized|tab|tablet|cap|capsule|soln|solution|susp|suspension|inj|injection)\b/g, " ")
      // remove common dose units
      .replace(/\b\d+(?:\.\d+)?\s*(mg|mcg|g|ml|l|unit|units|iu|meq|mmol)\b/g, " ")
      .replace(/[^a-z0-9가-힣]+/g, "") // strip spaces/punct
      .trim();
  }


  function resolveMedKey(patient, scannedName){
    if(!patient || !patient.meds) return null;

    const direct = String(scannedName||"").trim();
    if(direct && patient.meds[direct]) return direct;

    const target = normalizeMedName(direct);
    if(!target) return null;

    const keys = Object.keys(patient.meds);

    // 1) exact normalized match
    let matches = [];
    for(const k of keys){
      const nk = normalizeMedName(k);
      if(nk && nk === target) matches.push({k, nk});
    }
    if(matches.length === 1) return matches[0].k;

    // 2) containment match (handles 'aspirin' vs 'aspirinpo' etc.)
    matches = [];
    for(const k of keys){
      const nk = normalizeMedName(k);
      if(!nk) continue;
      if(nk.includes(target) || target.includes(nk)) matches.push({k, nk});
    }
    if(matches.length === 1) return matches[0].k;

    // 3) choose a clear winner by smallest length difference
    if(matches.length > 1){
      matches.sort((a,b)=> Math.abs(a.nk.length - target.length) - Math.abs(b.nk.length - target.length));
      const best = matches[0];
      const second = matches[1];
      if(best && second){
        const d1 = Math.abs(best.nk.length - target.length);
        const d2 = Math.abs(second.nk.length - target.length);
        if(d1 < d2) return best.k;
      }
    }
...
async function recordBatch(payload, raw){
    ...
    const batchPatient = (payload && payload.patient && typeof payload.patient === "object") ? payload.patient : null;
    const batchKey = batchPatient ? patientKey(batchPatient) : "";
    const ctxKey = patientKey(ctx.patient);
    ...
    // Batch QR에 환자정보가 포함된 legacy QR이더라도, 기본은 "현재 스캔된 환자(ctx.patient)" 기준으로 진행
    // 다만 payload.patient가 있고 ctx.patient와 다르면 차단 대신 경고 후 진행
    if(batchKey && ctxKey && ctxKey !== batchKey){
      // Reduce false positives: only warn when MRN is clearly different
      const bmrn = __normId(batchPatient?.mrn);
      const cmrn = __normId(ctx.patient?.mrn);
      if(bmrn && cmrn && bmrn !== cmrn){
        const ok = await showClickModal("Batch QR의 환자정보(MRN)가 현재 확인된 환자와 다릅니다. (Batch의 환자정보는 무시하고) 계속 기록할까요?");
        if(!ok){
          setStatus(scanStatus, "취소됨. 환자/Batch QR을 다시 확인하세요.");
          return;
        }
      }
    }

    const found = findPatientInState(ctx.patient);
    if(!found){
    ...
    const sched = normalizeHHMM(payload.time) || normalizeHHMM(nowHHMM()) || nowHHMM();
    const given = nowHHMM();
    const day = todayKey();
    ...
    let recorded = 0;
    let missing = 0;
    for(const scanned of meds){
      const medKey = resolveMedKey(patient, scanned);
      if(!medKey){
        missing++;
        continue;
      }
      const med = patient.meds[medKey];
      if(!med) { missing++; continue; }
      ...
      med.history[day].push({
        ...
        batchId,
        scannedName: String(scanned||"").trim()
      });
      recorded++;
    }
    // (persisted by MAR_APP.save() -> saveState() which updates __rev/__updatedAt)

    window.MAR_APP.setState(st);
    if(window.MAR_APP.save) window.MAR_APP.save();
    ctx.lastBatchId = batchId;
    ctx.lastBatchAt = Date.now();

    setStatus(scanStatus, `Batch 확인 됨 ✔ (${recorded}개 기록됨${missing?`, ${missing}개 매칭 실패`:``}) — 저장됨`);
  }
  ...
  async function handleDecoded(raw, source){
    ...
    if(payload.type === "patient"){
      ...
      // Set ctx from QR first
      ctx.patient = { name: String(p.name).trim(), room: String(p.room).trim(), mrn: String(p.mrn).trim() };

      // Immediately resolve to the exact patient key used in app state (patients are keyed by name)
      const found = findPatientInState(ctx.patient);
      if(!found){
        ctx.patient = null;
        ctx.patientSetAt = 0;
        ctx.mode = "patient";
        setStatus(scanStatus, "앱 상태에 해당 환자가 없습니다. (환자 리스트에 추가되었는지 / 이름·MRN·Room이 일치하는지 확인)");
        return;
      }

      // Lock to the state's patient key to avoid later 'name key mismatch'
      ctx.patient.name = found.name;

      ctx.patientSetAt = Date.now();
      ctx.mode = "batch";
      setStatus(scanStatus, `환자 확인 됨 ✔ ${ctx.patient.name} / ${ctx.patient.room} / ${ctx.patient.mrn}  (10분 유효)`);
      return;
    }

    if(payload.type === "batch"){
      if(!ctx.patient){
        setStatus(scanStatus, "먼저 환자 QR을 스캔하세요.");
        return;
      }
      const meds = Array.isArray(payload.meds) ? payload.meds : [];
      if(meds.length === 0){
        setStatus(scanStatus, "Batch QR에 meds(약 목록)이 없습니다.");
        return;
      }
