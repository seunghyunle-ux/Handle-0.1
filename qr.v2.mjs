...
function buildPayload({ facilityCode, patientName, patientRoom, patientMRN, time, meds }){
  // ✅ Batch QR는 환자 QR(팔찌) 스캔 후 기록하므로 환자정보를 QR에 넣지 않습니다.
  //    (길이↓, 스캔성공률↑, 환자 불일치 이슈↓)
  const payload = {
    v: 3,
    type: "batch",
...
for(const medName of meds){
          ...
          // ✅ per-med actions: delete this time / edit this time
          ...
          const delBtn = w.document.createElement("button");
          delBtn.type = "button";
          delBtn.className = "smallbtn";
          delBtn.textContent = "Delete";

          const editBtn = w.document.createElement("button");
          editBtn.type = "button";
          ...
          async function commitAndRebuild(mutator){
            const st2 = APP.getState();
            const p2 = st2 && st2.patients ? st2.patients[patientName] : null;
            if(!p2 || !p2.meds) return;
            mutator(p2);
            if(APP.commitState) APP.commitState(st2, "qr_popup_edit");
            else if(APP.setState) APP.setState(st2);

            // rebuild left list
            const refreshed = APP.getState();
            const p3 = refreshed && refreshed.patients ? refreshed.patients[patientName] : null;
            const groups2 = groupMedsByTime(p3);
            leftBody.innerHTML = "";
            if(groups2.length === 0){
              leftBody.innerHTML = `<div class="hint">등록된 약/시간이 없습니다. 먼저 +Med로 추가하세요.</div>`;
            }else{
              for(const [t2, meds2] of groups2){
                const g2 = w.document.createElement("div");
                g2.className = "group";
                g2.dataset.time = t2;

                const head2 = w.document.createElement("div");
                head2.className = "gtitle";
                head2.innerHTML = `<div>${escapeHtml(t2)}</div><div class="hint">${escapeHtml(String(meds2.length))} meds</div>`;
                ...
                for(const med2 of meds2){
                  ...
                  const l2 = w.document.createElement("label");
                  l2.className = "chk";
                  l2.innerHTML = `<input type="checkbox" data-time="${escapeHtml(t2)}" data-med="${escapeHtml(med2)}" />
                                  <span class="medname">${escapeHtml(med2)}</span>`;

                  const a2 = w.document.createElement("div");
                  a2.style.display = "flex";
                  a2.style.flexDirection = "column";
                  a2.style.gap = "6px";
                  a2.style.alignItems = "stretch";

                  const d2 = w.document.createElement("button");
                  d2.type = "button";
                  d2.className = "smallbtn";
                  d2.textContent = "Delete";

                  const e2 = w.document.createElement("button");
                  e2.type = "button";
                  e2.className = "smallbtn";
                  e2.textContent = "Edit";
                  ...
                  d2.onclick = async ()=>{
                    const ok = confirm(`"${med2}" (${t2}) 이 시간을 삭제할까요?`);
                    if(!ok) return;
                    await commitAndRebuild((pp)=>{
                      const md = pp.meds[med2];
                      if(!md) return;
                      md.times = (md.times||[]).filter(x=>String(x)!==String(t2));
                      if(!md.times || md.times.length===0) delete pp.meds[med2];
                    });
                  };

                  e2.onclick = async ()=>{
                    const next = prompt(`"${med2}" 시간을 변경하세요 (HH:MM)`, String(t2));
                    if(next===null) return;
                    const nt = String(next).trim();
                    if(!isHHMM(nt)){
                      alert("시간 형식은 HH:MM 입니다. 예) 08:00");
                      return;
                    }
                    await commitAndRebuild((pp)=>{
                      const md = pp.meds[med2];
                      if(!md) return;
                      const arr = (md.times||[]).map(x=>String(x));
                      const out = arr.map(x=> x===String(t2) ? nt : x);
                      md.times = [...new Set(out)].sort((a,b)=>a.localeCompare(b));
                    });
                  };
                }
                leftBody.appendChild(g2);
              }
            }
          }

          delBtn.onclick = async ()=>{
            const ok = confirm(`"${medName}" (${time}) 이 시간을 삭제할까요?`);
            if(!ok) return;
            await commitAndRebuild((pp)=>{
              const md = pp.meds[medName];
              if(!md) return;
              md.times = (md.times||[]).filter(x=>String(x)!==String(time));
              if(!md.times || md.times.length===0) delete pp.meds[medName];
            });
          };

          editBtn.onclick = async ()=>{
            const next = prompt(`"${medName}" 시간을 변경하세요 (HH:MM)`, String(time));
            if(next===null) return;
            const nt = String(next).trim();
            if(!isHHMM(nt)){
              alert("시간 형식은 HH:MM 입니다. 예) 08:00");
            ...
            await commitAndRebuild((pp)=>{
              ...
              if(!md) return;
              const arr = (md.times||[]).map(x=>String(x));
              const out = arr.map(x=> x===String(time) ? nt : x);
              md.times = [...new Set(out)].sort((a,b)=>a.localeCompare(b));
            });
          };
        }
...
function setStatus(el, msg){
  ...
  // High-visibility pill styling (minimal, no layout changes)
  const m = String(msg||"");
  ...
  const isOk = m.includes("✓") || m.includes("✔") || /완료|확인 됨/i.test(m);
  const isErr = m.includes("❌") || /불일치|다릅니다|없습니다|실패|만료|권한/i.test(m);
...
async function initScanUI(){
  ...
  let scanDlg = document.getElementById("scanDlg");
  if(!scanBtn) return;

  // ✅ scanDlg 마크업이 HTML에 없으면(통합 과정에서 빠진 경우) 여기서 자동 생성
  if(!scanDlg){
    scanDlg = document.createElement("dialog");
    scanDlg.id = "scanDlg";
    scanDlg.style.maxWidth = "720px";
    scanDlg.style.width = "calc(100% - 24px)";
    scanDlg.innerHTML = `
      <div style="display:flex; flex-direction:column; gap:10px; padding:14px;">
        <div style="display:flex; justify-content:space-between; align-items:center; gap:10px;">
          ...
          <button class="btn secondary" id="scanCloseBtn" type="button">Close</button>
        </div>

        <div id="scanStatus" style="font-weight:800;">환자 QR을 스캔하세요.</div>

        <div style="display:flex; gap:10px; flex-wrap:wrap; align-items:center;">
          <input id="scanInput" placeholder="스캐너 입력 후 Enter" style="flex:1; min-width:240px; padding:10px 12px; border-radius:12px; border:1px solid rgba(0,0,0,.15);" />
          <button class="btn" id="camStartBtn" type="button">Camera Start</button>
          <button class="btn secondary" id="camStopBtn" type="button" disabled>Stop</button>
          <button class="btn secondary" id="camNextBtn" type="button" disabled>Next scan</button>
        </div>

        <div id="scanReader" style="width:100%; min-height:260px; border-radius:14px; border:1px solid rgba(0,0,0,.12); overflow:hidden; background:#fff;"></div>

        <div style="color:#666; font-size:12px; line-height:1.35;">
          순서: <b>환자 QR</b> → (자동으로 해당 환자 선택) → <b>Batch QR</b><br/>
          환자 확인은 10분간 유효합니다.
        </div>
      </div>
    `;
    document.body.appendChild(scanDlg);
  }
async function refreshScanButtonAccess(){
  ...
  // Prevent "aim hint" from overwriting success/error messages immediately
  let __statusFreezeUntil = 0;
  let __lastAimHintAt = 0;
  function freezeStatus(ms){ __statusFreezeUntil = Date.now() + (ms || 0); }

  function resetScanStabilizer(){
    __candidateText = "";
    __candidateAt = 0;
    __acceptLock = false;
  }
    ...
    await camera.start(
      ...
      async (decodedText)=>{
        ...
        if(t && t === __candidateText && (now - __candidateAt) <= 900){
          // ✅ Confirmed scan (seen twice). DO NOT stop the camera here.
          // Stopping/restarting the camera causes "flicker" on mobile and hurts scan success.
          __acceptLock = true;
          try{
            await handleDecoded(t, "camera");
            // Prevent immediate re-processing of the same QR
            __candidateText = "";
            __candidateAt = 0;
            camNextBtn.disabled = false; // optional: acts like "다음/확인"
          } finally {
            // Allow next scan after a short cooldown
            setTimeout(()=>{ __acceptLock = false; }, 1200);
          }
          return;
        }
        ...
        __candidateText = t;
        __candidateAt = now;
        if(now >= __statusFreezeUntil && (now - __lastAimHintAt > 700)){
          setStatus(scanStatus, "QR 조준 유지…");
          __lastAimHintAt = now;
        }
      },
  ...
  async function recordBatch(payload, raw){
    ...
    // Batch QR는 patient 객체가 없을 수 있습니다. (pid만 사용)
    const pid = payload && payload.pid ? String(payload.pid) : "";
    const key = patientKey(ctx.patient);
    ...
    if(pid){
      const expectedPid = patientIdFromName(ctx.patient?.name);
      if(String(pid) !== String(expectedPid)){
        setStatus(scanStatus, "환자가 다릅니다 ❌ (Batch가 다른 환자용입니다)");
        ctx.mode = "patient";
        ctx.patient = null;
        ctx.patientSetAt = 0;
        resetScanStabilizer();
    ...
    const found = findPatientInState({ name: ctx.patient?.name });
    if(!found){
  ...
  async function handleDecoded(raw, source){
    ...
    if(payload.type === "patient"){
      ...
      // ✅ 스캔만으로 해당 환자로 MAR 화면 이동
      try{ if(window.MAR_APP && window.MAR_APP.selectPatient){ window.MAR_APP.selectPatient(String(p.name).trim()); } }catch(_e){}
      ctx.patientSetAt = Date.now();
      ctx.mode = "batch";
      setStatus(scanStatus, `환자 스캔 완료 ✓ ${ctx.patient.name} / ${ctx.patient.room} / ${ctx.patient.mrn}  (10분 유효)`);
      // keep camera running; just switch mode + allow immediate next scan
      resetScanStabilizer();
      freezeStatus(1400);
      camNextBtn.disabled = false;
      return;
    }
