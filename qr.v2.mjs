...
function setStatus(el, msg){
  ...
  // Make status highly visible (green for success, red for error)
  const s = String(msg || "");
  let kind = "info";
  if(/[✓✔]/.test(s)) kind = "ok";
  if(/[✗❌]|ERROR|ERR|실패|불일치|없습니다|만료/.test(s)) kind = "err";
...
if(recorded === 0){
  setStatus(scanStatus, "약이 다릅니다 ❌ (이 Batch의 약이 환자 MAR에 없습니다)");
  return;
}
    ...
    setStatus(scanStatus, `약 스캔 완료 ✓ (${recorded}개 약 기록됨)`);
  }
