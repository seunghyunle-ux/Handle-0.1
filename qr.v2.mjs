...
(function renderPatientIdQr(){
  ...
  patientQrCard.appendChild(card);
    ...
    // ✅ Export 체크박스(환자 QR 포함) 변경 시 항상 Export 버튼 상태 갱신
    rightBody.addEventListener("change", (e)=>{
      const t = e.target;
      if(t && t.matches && t.matches('input[type="checkbox"][data-export="1"]')){
        updateExportEnabled();
      }
    });
...
function parsePayload(raw){
  ...
  // Try URL query payloads: ...?payload=<json or b64> / ?data=...
  if(!o){
    try{
      if(/^https?:\/\//i.test(t)){
        const u = new URL(t);
        const keys = ["payload","data","qr","q","json","p"];
        for(const k of keys){
          const v = u.searchParams.get(k);
          if(!v) continue;
          o = tryJson(v) || tryJson(safeDecodeURIComponent(v));
          if(o) break;
          if(/^b64:/i.test(v)){
            try{ o = tryJson(atob(v.slice(4))); }catch(_e){}
            if(o) break;
          }
        }
      }
  ...
  if(!o){
    const d = safeDecodeURIComponent(t);
    if(d !== t) o = tryJson(d);
  }
  ...
  if(!o){
    if(/^b64:/i.test(t)){
      try{ o = tryJson(atob(t.slice(4))); }catch(_e){}
    }else if(/^[A-Za-z0-9+/=]{40,}$/.test(t)){
      try{ o = tryJson(atob(t)); }catch(_e){}
    }
  }
...
async function initScanUI(){
  ...
  async function refreshScanButtonAccess(){
    try{
      const role = await getUserRole((window.__MAR_FB__||{}).auth,(window.__MAR_FB__||{}).db);
  ...
  async function handleDecoded(raw, source){
    ...
    if(!payload){
      // Fallback: many wristband QR codes are just MRN (plain text).
      // If it matches a patient MRN in local state, treat it as a patient scan.
      const mrn = String(raw||"").trim();
      if(mrn && window.MAR_APP && typeof window.MAR_APP.getState === "function"){
        try{
          const st = window.MAR_APP.getState();
          const patients = (st && st.patients) ? st.patients : {};
          let found = null;
          for(const name of Object.keys(patients)){
            const p = patients[name] || {};
            if(String(p.mrn||"").trim() === mrn){
              found = { name: String(name).trim(), room: String(p.room||"").trim(), mrn };
              break;
            }
          }
          if(found){
            ...
            ctx.mode = "batch";
            setStatus(scanStatus, `환자 확인 됨 ✔ ${found.name} / ${found.room||"-"} / ${found.mrn}  (10분 유효)`);
            return;
          }
        }catch(_e){}
      }
