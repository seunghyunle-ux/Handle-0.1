...
if(patientExpired()){
    ...
    if(patientKey(ctx.patient) !== key){
      ...
      // mismatch → stay in patient scan mode (camera keeps running)
      ctx.mode = "patient";
      ctx.patient = null;
      ctx.patientSetAt = 0;
      resetScanStabilizer();
...
(async ()=>{
  try{
    ...
    // ✅ Batch QR에 room/mrn이 들어있더라도 비교하지 않습니다. (표기/공백 차이로 불일치가 자주 발생)
    // 안전을 위해 "이름"만 존재할 때만 비교하고, 이름이 없으면 비교를 건너뜁니다.
    const scannedName = String(ctx.patient?.name || "").trim().toLowerCase();
    const batchName = String(p.name || "").trim().toLowerCase();
    if(batchName && scannedName && batchName !== scannedName){
