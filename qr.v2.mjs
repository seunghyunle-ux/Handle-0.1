...
function buildPayload({ facilityCode, patientName, patientRoom, patientMRN, time, meds }){
  // ✅ Batch QR는 "환자 스캔(팔찌)" 이후에만 사용하므로, room/mrn/name 전체를 넣지 않고
  //    안정적인 patientId(pid)만 넣어 길이를 줄이고 불필요한 불일치를 방지합니다.
  const payload = {
    v: 2,
    type: "batch",
    facility: facilityCode || null,
    pid: patientIdFromName(patientName),
    time: time || null,
    meds: meds || []
  };
  // QR에는 URL 대신 JSON 자체를 넣습니다. (길이↓, 스캔 성공률↑)
