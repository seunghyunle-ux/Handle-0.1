...
function parsePayload(raw){
  ...
  // html5-qrcode typically returns a string, but be defensive.
  let s = (typeof raw === "string") ? raw : String(raw);
  s = s.trim();
  if(!s) return null;

  // Some scanners wrap the whole JSON as a JSON string: ""{...}""
  // Also some scanners prefix text (e.g. "QR:" / "MAR:") or embed JSON in a URL.
  const tryCandidates = [];
  tryCandidates.push(s);

  // If URL, try query params (payload/data/qr) or hash.
  try{
    if(/^https?:\/\//i.test(s)){
      const u = new URL(s);
      const keys = ["payload","data","qr","q","text","value"];
      for(const k of keys){
        const v = u.searchParams.get(k);
        if(v) tryCandidates.push(v);
      }
      if(u.hash && u.hash.length > 1) tryCandidates.push(u.hash.slice(1));
    }
  }catch(_e){}

  // If it contains percent-encoding, try decoding.
  if(s.includes("%")){
    try{ tryCandidates.push(decodeURIComponent(s)); }catch(_e){}
  }

  // If it looks like it contains JSON somewhere, extract the {...} part.
  const firstBrace = s.indexOf("{");
  const lastBrace = s.lastIndexOf("}");
  if(firstBrace !== -1 && lastBrace !== -1 && lastBrace > firstBrace){
    tryCandidates.push(s.slice(firstBrace, lastBrace+1));
  }

  // If it is prefixed base64 like "b64:...."
  if(/^b64:/i.test(s)){
    try{
      const b = s.replace(/^b64:/i,"").trim();
      tryCandidates.push(atob(b));
    }catch(_e){}
  }

  // Try parse each candidate. If parse result is a string that itself is JSON, parse again.
  for(const cand0 of tryCandidates){
    let cand = (cand0 == null) ? "" : String(cand0).trim();
    if(!cand) continue;

    // decode URI encoding if needed (e.g. %7B...%7D)
    if(cand.includes("%")){
      try{ cand = decodeURIComponent(cand); }catch(_e){}
    }

    // Try JSON parse once/twice
    try{
      let o = JSON.parse(cand);
      if(typeof o === "string"){
        const inner = o.trim();
...
async function initScanUI(){
  ...
  async function recordBatch(payload, raw){
    ...
    if(patientKey(ctx.patient) !== key){
      setStatus(scanStatus, "환자 불일치 ❌ (이름/MRN/방번호 확인)");
      return;
    }
