...
function buildPayload({ facilityCode, time, meds }){
  // ✅ Batch QR에는 "약 이름"만 포함 (환자 정보는 포함하지 않음)
  //    환자 확인은 "환자 QR"을 먼저 스캔해서 ctx.patient로 확보한 뒤 진행.
  //    시간 불일치 경고는 payload.time으로 판단 가능.
  const payload = {
    v: 1,
    type: "batch",
...
async function initScanUI(){
  ...
  async function recordBatch(payload, raw){
    ...
    // ✅ Batch QR에는 환자정보가 없을 수 있음 (약 이름만 포함).
    //    다만, legacy/호환을 위해 payload.patient가 있으면 ctx.patient와 일치하는지 확인.
    if(payload.patient){
      const key = patientKey(payload.patient);
      if(key && patientKey(ctx.patient) !== key){
    ...
    // ✅ 시간 불일치면 즉시 경고(클릭/터치로만 진행)
    if(payload.time && !timeOk(payload.time)){
      const ok = await showClickModal(`시간 불일치 ⚠️ (스케줄 ${sched} / 현재 ${given}). 그래도 기록할까요?`);
      if(!ok){
        setStatus(scanStatus, "취소됨. 다음 Batch를 스캔하세요.");
        return;
      }
    ...
    // late면 상태 메시지에도 표시
    const lateMsg = (!timeOk(payload.time) && payload.time) ? " (시간 불일치 기록됨)" : "";
    setStatus(scanStatus, `Batch 확인 됨 ✔ (${recorded}개 약 기록됨)${lateMsg}`);
  }
