...
async function initScanUI(){
  ...
  async async function recordBatch(payload, raw){
    ...
    // bump top-level metadata for sync guard
    st._rev = (Number(st._rev) || 0) + 1;
    ...
    setStatus(scanStatus, `Batch 확인 됨 ✔ (${recorded}개 약 기록됨)`);
  }

  async async function handleDecoded(raw, source){
